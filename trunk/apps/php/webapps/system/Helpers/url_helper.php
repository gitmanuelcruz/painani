...
/*-----------URL PARA LOS INFORMES------------*/
if (! function_exists('_get_uri_info')) {
   function _get_uri_info(string $relativePath = '', ? App $config = null): URI
   {
      $config ??= config('App');
      if ($config->infoURL === '') {
         throw new InvalidArgumentException('_get_uri_info() requires a valid infoURL.');
      }
...
if (! function_exists('site_url_info')) {
   function site_url_info($relativePath = '', ?string $scheme = null, ? App $config = null): string
   {
...
if (! function_exists('info_url')) {
    ...
    * @param string $scheme
    */
   function info_url($relativePath = '', ? string $scheme = null): string
   {
      $config            = clone config('App');
      $config->indexPage = '';

      return rtrim(site_url_info($relativePath, $scheme, $config), '/');
   }
}
/*----------------------------------------------------------------------------*/
